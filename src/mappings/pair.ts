...
import { TradingComp } from "../../generated/templates";
...
export function handleSwap(event: Swap): void {
  ...
  if (userComp !== null) {
      log.info("PARTICIPATED TRADE ::userComp !== null:: {} || {}", [
        event.transaction.from
          .toHexString()
          .concat("_")
          .concat(tradingCompAddress),
        userComp.totalTxns.toString(),
      ]);
    ...
    if (event.block.timestamp < tradingComp.endTime) {
      ...
      if (pairContract !== null) {
        ...
        userComp.totalTxns = userComp.totalTxns.plus(ONE_BI);
        userComp.save();
